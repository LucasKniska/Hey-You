...
def embed_user(user: User):
    raw = build_paragraph(user)
    _, vectors = cleaner.process_user_profile(raw)
    return vectors

def create_new_match(user_a: User, user_b: User, similarity: float):
    match_doc = {
        "score": similarity,
        "status": "NEW",
        "userData": [
            {"id": user_a.id, "response": "AWAITING"},
            {"id": user_b.id, "response": "AWAITING"}
        ]
    }
    match_ref = db.collection(const.NEW_MATCHES).document()
    match_ref.set(match_doc)

    # set CurrentMatch on each user
    db.collection(const.USERS).document(user_a.id).update({"CurrentMatch": match_ref.id})
    db.collection(const.USERS).document(user_b.id).update({"CurrentMatch": match_ref.id})

# --------------------------------------------------
@app.post("/auto-match/{user_id}")
def auto_match(user_id: str):
    """Pull `user_id`, compute best match among discoverable users."""
    # 1) fetch the requesting user
    me_ref = db.collection(const.USERS).document(user_id)
    me_snap = me_ref.get()
    if not me_snap.exists:
        raise HTTPException(404, "User not found")

    me = User.from_json(me_snap.to_dict())

    # 2) pre-check: already matched?
    if me.currentMatch:
        return {"status": "already_matched", "match_id": me.currentMatch}

    # 3) compute my embedding once
    me_vec = embed_user(me)

    # 4) scan other discoverable users
    others = (
    ...
    for snap in others:
        if snap.id == me.id:
            continue
        cand = User.from_json(snap.to_dict())

        # skip if candidate is already matched
        if cand.currentMatch:
            continue

        cand_vec = embed_user(cand)
        score = dist.profile_similarity(me_vec, cand_vec)
        if score > best_score:
            best_other, best_score = cand, score

    if best_other is None:
        return {"status": "no_candidate_found"}

    # 5) create a match doc & update both users
    create_new_match(me, best_other, best_score)

    return {
        "status": "match_created",
        "match_user_ids": [me.id, best_other.id],
